...
print("Train a CLIP model on Flickr8k")
...
checkpoint_path = clip._download(
    _VIT_B_32_URL, os.path.expanduser("~/.cache/clip")
)
...
for k, v in pretrained_state.items():
    if k in model_state:
        if model_state[k].shape == v.shape:
            ...
            skipped_keys.append((k, "shape mismatch", model_state[k].shape, v.shape))
    else:
        skipped_keys.append((k, "missing in model"))
...
print(f"Pretrained init: {matched_keys.__len__} tensors matched/loaded, {skipped_keys.__len__()} skipped")
